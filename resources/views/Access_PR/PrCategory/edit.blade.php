    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Edit Kategori PR: ') . $prCategory->name }}
            </h2>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('pr-categories.update', $prCategory) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <textarea name="description" 
                                      ...
                                      rows="3" 
                                      placeholder="Deskripsi kategori ini (opsional)">{{ old('description', $prCategory->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                @foreach($approvalLevels as $key => $label)
                                    <div class="flex items-center p-3 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                        <input type="checkbox" 
                                               ...
                                               {{ in_array($key, old('approval_rules', $prCategory->approval_rules)) ? 'checked' : '' }}>
                        ...
                        @if($prCategory->purchaseRequests()->count() > 0)
                            <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg p-4">
                                ...
                                <p class="text-sm text-yellow-800 dark:text-yellow-200">
                                    Kategori ini digunakan oleh {{ $prCategory->purchaseRequests()->count() }} Purchase Request. 
                                    Perubahan aturan persetujuan hanya akan berlaku untuk PR baru yang dibuat setelah ini.
                                </p>
                            </div>
                        @endif
