    ...
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                ...
                <a href="{{ route('admin.configuration.divisi.index') }}" class="text-blue-600 hover:underline">Master Divisi</a> / Edit
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6">
                <form action="{{ route('admin.configuration.divisi.update', $divisi) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="kode" id="kode" value="{{ old('kode', $divisi->kode) }}" required
                        ...
                        @if($divisi->isUsedByUsers())
                            <p class="mt-1 text-xs text-amber-600 dark:text-amber-400">⚠️ Divisi ini digunakan oleh {{ $divisi->getUsersCount() }} user. Hati-hati mengubah kode!</p>
                        @endif
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3"
                                  class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $divisi->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('admin.configuration.divisi.index') }}" 
